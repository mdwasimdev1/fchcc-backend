(function ($) {
	...
	var icDashboard = function () {
		...
		var chartSpendingStatistic = function () {
			if ($("#chartSpendingStatistic").length > 0) {

				$.get('/admin/user-chart', function (response) {
		...
		$.get('/admin/user-totalUserGrowth', function (data) {
			...
			if (data.percentage > 0) {
				$("#totalPercent")
					...
					.html('<i class="fa-solid fa-arrow-up me-1"></i> +' + data.percentage + '%');

			} else if (data.percentage < 0) {
				$("#totalPercent")
					.removeClass('text-success text-secondary')
					.addClass('text-danger')
					.html('<i class="fa-solid fa-arrow-down me-1"></i> ' + data.percentage + '%');
		...
		$.get('/admin/user-growth', function (data) {

			$("#currentUsers").text(data.current);
		...
		$.get('/admin/user-status-growth', function (data) {


			$("#activeCount").text(data.active.current);

			if (data.active.percentage >= 0) {
				$("#activePercent")
					.removeClass('text-danger')
					.addClass('text-success')
					.text('+' + data.active.percentage + '%');
			} else {
				$("#activePercent")
					.removeClass('text-success')
					.addClass('text-danger')
			...
			$("#inactiveCount").text(data.inactive.current);

			if (data.inactive.percentage >= 0) {
				$("#inactivePercent")
					.removeClass('text-success')
					.addClass('text-danger')
					.text('+' + data.inactive.percentage + '%');
			} else {
				$("#inactivePercent")
					.removeClass('text-danger')
					.addClass('text-success')
		...
		var chartUser = function () {
			if (jQuery("#chartUser").length > 0) {
				var options = {
					series: [
						{
							name: 'Income',
